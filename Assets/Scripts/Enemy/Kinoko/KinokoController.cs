...
public class KinokoController : EnemyController
{
    ...
    KinokoDataSO convertData;
    float spawnElapsedTime = 0f;    // このキノコが生成されてからの経過時間
    bool hasTriggeredBreedAnimation = false;

    public KinokoDataSO ConvertData()
    {
        KinokoDataSO kinokoDataSO = Data as KinokoDataSO;
        if (kinokoDataSO != null)
        {
            convertData = kinokoDataSO;
        }
        else
        {
            Debug.LogError("Data is not of type KinokoDataSO. Please check the assigned EnemyDataSO.");
        }
    ...
    public void Initialize(Vector2 borderLinePos, Vector2 startPos, float enemyStatusMultiplier, bool isSpawnedBySpore)
    {
        ...
        if(convertData == null) ConvertData();
    }
    ...
    protected override void Update()
    {
        ...
        if (IsFilled && IsSpawnedBySpore)       // このキノコが胞子によって生成され、埋まった状態の場合
        {
            ...
            if (spawnElapsedTime >= convertData.FillDuration)
            {
        ...
        {
            spawnElapsedTime += Time.deltaTime;
            if(spawnElapsedTime >= convertData.BreedDelay)
            {
                ...
                if(spawnElapsedTime >= convertData.BreedDelay + convertData.BreedAnimationDuration)
                {
                    Instantiate(convertData.SporeParticle, transform.position, Quaternion.identity);    // 胞子のパーティクルを生成
                    // 繁殖処理を行う
                    for (int i = 0; i < convertData.SpawnCount; i++)
                    {
